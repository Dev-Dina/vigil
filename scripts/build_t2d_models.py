...
def _print_stratum(title: str, per_stratum: dict[str, dict[str, dict[str, float]]]) -> None:
...
def main() -> int:
    ...
    print(f"  Logit PR-AUC={_fmt(real['logit']['pr_auc'])} Brier={_fmt(real['logit']['brier'])}")
    ...
    print(f"  Logit PR-AUC={_fmt(sb['logit']['pr_auc'])} Brier={_fmt(sb['logit']['brier'])}")
    _print_stratum("1b GBT", sb["per_stratum_gbt"])
    print("-" * 78)

    pre = res["preregistration"]
    print("TASK 2 — PRE-REGISTERED BAR (written before the fit):")
    print(f"  PR-AUC must reach >= {pre['criteria']['pr_auc']['must_reach']} "
          f"(1b={pre['criteria']['pr_auc']['baseline']} + {pre['criteria']['pr_auc']['margin']})")
    print(f"  median lead-time must reach >= {pre['criteria']['lead_time']['must_reach']} visits")
    print("-" * 78)
    ...
    print(f"  PR-AUC  {_fmt(bar['sequence_test_pr_auc'])} >= {bar['must_reach_pr_auc']} "
          f"-> {'PASS' if bar['pr_auc_pass'] else 'FAIL'}")
    print(f"  lead    {_fmt(bar['sequence_median_lead_time_visits'])} >= {bar['must_reach_lead_time']} "
